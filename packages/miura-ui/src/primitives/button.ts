// miura-ui: primitives/button.ts
// Mobile-first, token-driven button for MiuraJS
...
export class MuiButton extends MiuraElement {
  ...
  static properties = {
    variant: { type: String, reflect: true, default: 'solid' },
    size: { type: String, reflect: true, default: 'md' },
    tone: { type: String, reflect: true, default: 'primary' },
  ...
  static get styles() {
    return css`
      ...
      .icon-slot { display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; }
      .label { display: inline-flex; align-items: center; justify-content: center; min-width: 0; }
      ::slotted([slot='icon-start']), ::slotted([slot='icon-end']) {
      ...
      :host([disabled]) button, button:disabled {
        cursor: not-allowed;
        opacity: 0.6;
        box-shadow: none;
      }
  ...
  template() {
    ...
    return html`
      ...
      >
        ${this.loading ? html`<span class="loader" aria-hidden="true"></span>` : ''}
